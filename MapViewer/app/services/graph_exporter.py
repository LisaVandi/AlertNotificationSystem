...
def get_graph_json(floor_level: int, 
                   image_filename: str, 
                   image_width: int, 
                   image_height: int, 
    ...
    try:
        with closing(psycopg2.connect(**DATABASE_CONFIG)) as conn, conn.cursor() as cur:
            # Fetch nodes
            cur.execute("""
                SELECT node_id, (x1 + x2) / 2 AS x, (y1 + y2) / 2 AS y, node_type, current_occupancy
                FROM nodes
                WHERE floor_level = %s
            """, (floor_level,))
            nodes = [{"id": row[0], "x": row[1], "y": row[2], "node_type": row[3], "current_occupancy": row[4]} for row in cur.fetchall()]
