...
def send_evacuation_path_to_user_simulator(rabbitmq_handler: RabbitMQHandler, message: Dict[str, Any]) -> None:
    try:
        rabbitmq_handler.send_message(
            exchange="",
        ...
        logger.info(f"Evacuation path sent to User Simulator for user_id = {message.get('user_id')}: {message}")
    except Exception as e:
